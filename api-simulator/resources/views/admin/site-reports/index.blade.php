...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Site Reports</h1>
            <p class="text-sm text-gray-500 mt-1">Monthly performance breakdown by site — impressions, clicks, conversions, earnings, CTR & ECPM.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
            <form method="GET" action="{{ route('admin.reports.site') }}" class="flex items-center gap-2 flex-1 flex-wrap">
                <div class="relative flex-1 max-w-xs">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by site name, domain, publisher..."
                ...
                @if(request()->hasAny(['search','site_id','publisher_id','start_month','end_month']))
                    <a href="{{ route('admin.reports.site') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-500 hover:bg-gray-50 transition-colors">Clear</a>
                @endif
            </form>
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-44 bg-white rounded-xl border border-gray-200 shadow-lg py-1 z-50" style="display:none;">
                    ...
                    <a href="{{ route('admin.reports.site.export', request()->all()) }}" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">CSV</a>
        ...
        <div class="overflow-x-auto">
            <table id="siteReportsTable" class="w-full text-sm">
                <thead>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($reports as $row)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                <div class="font-medium text-gray-900">{{ $row->site_name }}</div>
                                <div class="text-xs text-gray-400">{{ $row->site_domain }}</div>
                                @php $sc = match($row->site_status ?? '') { 'active' => 'bg-emerald-100 text-emerald-700', 'pending' => 'bg-amber-100 text-amber-700', default => 'bg-gray-100 text-gray-500' }; @endphp
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold uppercase {{ $sc }} mt-0.5">{{ $row->site_status ?? 'unknown' }}</span>
                            </td>
                            <td class="px-4 py-3">
                    ...
                    @empty
                        <tr><td colspan="9" class="px-4 py-12 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p class="text-sm text-gray-500">No site report data found.</p>
                            </div>
                        </td></tr>
                    @endforelse
                </tbody>
...
<script>
function copyTable() {
    const rows = document.getElementById('siteReportsTable').querySelectorAll('tr');
    let text = '';
