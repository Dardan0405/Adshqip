...
<div class="bg-white rounded-xl border border-gray-200">
            ...
            <div class="flex items-center gap-2">
                <div class="relative" x-data="{ open: false }">
                    ...
                    <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-44 bg-white rounded-xl border border-gray-200 shadow-lg py-1 z-50" style="display: none;">
                        <a href="{{ route('admin.adformats.reports.export', array_merge(['id' => $ad['id'], 'format' => 'csv'], request()->only(['date_from', 'date_to', 'device', 'country']))) }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
                        ...
                        <a href="{{ route('admin.adformats.reports.export', array_merge(['id' => $ad['id'], 'format' => 'excel'], request()->only(['date_from', 'date_to', 'device', 'country']))) }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
                        ...
                        <a href="{{ route('admin.adformats.reports.export', array_merge(['id' => $ad['id'], 'format' => 'pdf'], request()->only(['date_from', 'date_to', 'device', 'country']))) }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
    ...
    <div class="px-6 py-3 border-b border-gray-100 bg-gray-50/50">
        <form method="GET" action="{{ route('admin.adformats.reports', $ad['id']) }}" class="flex flex-col md:flex-row md:items-center gap-3">
            {{-- Date From --}}
            <div class="flex items-center gap-2">
            ...
            @if($dateFrom !== now()->subDays(29)->format('Y-m-d') || $dateTo !== now()->format('Y-m-d') || $deviceFilter !== 'all' || $countryFilter !== 'all')
                <a href="{{ route('admin.adformats.reports', $ad['id']) }}" class="px-3 py-2 rounded-lg border border-gray-200 text-sm text-gray-500 hover:bg-gray-50 transition-colors">Clear</a>
            @endif
        </form>
    </div>
        ...
        <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
            ...
            <div class="flex items-center gap-1">
                @if($pagination['current_page'] > 1)
                    <a href="{{ route('admin.adformats.reports', array_merge(['id' => $ad['id']], request()->query(), ['page' => $pagination['current_page'] - 1])) }}" class="px-3 py-1.5 rounded-lg border border-gray-200 text-xs text-gray-600 hover:bg-gray-50 transition-colors">Prev</a>
                @endif

                @for($p = max(1, $pagination['current_page'] - 2); $p <= min($pagination['total_pages'], $pagination['current_page'] + 2); $p++)
                    <a href="{{ route('admin.adformats.reports', array_merge(['id' => $ad['id']], request()->query(), ['page' => $p])) }}" class="px-3 py-1.5 rounded-lg text-xs {{ $p === $pagination['current_page'] ? 'bg-brand-600 text-white' : 'border border-gray-200 text-gray-600 hover:bg-gray-50' }} transition-colors">{{ $p }}</a>
                @endfor

                @if($pagination['current_page'] < $pagination['total_pages'])
                    <a href="{{ route('admin.adformats.reports', array_merge(['id' => $ad['id']], request()->query(), ['page' => $pagination['current_page'] + 1])) }}" class="px-3 py-1.5 rounded-lg border border-gray-200 text-xs text-gray-600 hover:bg-gray-50 transition-colors">Next</a>
                @endif
            </div>
        </div>
    @endif
