...
@section('title', 'Publisher Payment Approvals')
...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Publisher Payment Approvals</h1>
            <p class="text-sm text-gray-500 mt-1">Review publisher payment invoices with payout-style approvals and detail views.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        @php
            $statCards = [
                ...
                ['label' => 'Approved', 'value' => number_format($paidCount), 'color' => 'text-blue-700', 'bg' => 'bg-blue-50', 'border' => 'border-blue-200', 'icon' => '<path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>'],
    ...
    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
            <form method="GET" action="{{ route('admin.publisher-payment-approvals') }}" class="flex items-center gap-2 flex-1 flex-wrap">
                <div class="relative flex-1 min-w-[220px] max-w-md">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search id, email, invoice id..."
                ...
                <select name="publisher_id" class="px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20">
                ...
                <a href="{{ route('admin.publisher-payment-approvals') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50">Reset</a>
            </form>
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-44 bg-white rounded-xl border border-gray-200 shadow-lg py-1 z-50" style="display:none;">
                    ...
                    <a href="{{ route('admin.publisher-payment-approvals.export', request()->all()) }}" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">CSV</a>
        ...
        <div class="overflow-x-auto">
            <table id="publisherPaymentApprovalsTable" class="w-full text-sm">
                <thead>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($invoices as $invoice)
                        ...
                        @endphp
                        <tr class="hover:bg-gray-50/50 transition-colors" id="row-{{ $invoice->id }}">
                        ...
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-sm text-gray-500">No publisher payment approvals found.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg z-10">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <h3 class="text-lg font-bold text-gray-900">Publisher Payment Details</h3>
...
<script>
    function viewDetails(id) {
        ...
        fetch(`/admin/publisher-payment-approvals/${id}`, {
            ...
            content.innerHTML = `
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <div class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Invoice Id</div>
                            <div class="text-sm font-medium text-gray-900">${data.invoice_id}</div>
                        </div>
                        <div>
                            <div class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Invoice Status</div>
                            <div class="text-sm text-gray-900">${data.status}</div>
                        </div>
                        <div>
                            <div class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Invoice Date</div>
                            <div class="text-sm text-gray-700">${data.invoice_date}</div>
                        </div>
                        <div>
                            <div class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Due Date</div>
                            <div class="text-sm text-gray-700">${data.due_date || 'N/A'}</div>
                        </div>
                        <div>
                            <div class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Amount</div>
                            <div class="text-sm text-gray-700">EUR ${data.amount}</div>
                        </div>
                        <div>
                            <div class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Tax</div>
                            <div class="text-sm text-gray-700">EUR ${data.tax_amount}</div>
                        </div>
                        <div class="col-span-2">
                            <div class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Invoice Amount</div>
                            <div class="text-sm font-semibold text-emerald-700">EUR ${data.total_amount}</div>
                        </div>
                        <div class="col-span-2">
                            <div class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Paid At</div>
                            <div class="text-sm text-gray-700">${data.paid_at || 'Not paid yet'}</div>
                        </div>
                    </div>
                </div>
            `;
        ...
        .catch(() => {
            content.innerHTML = '<p class="text-sm text-red-500 text-center py-8">Failed to load publisher payment details.</p>';
        });
    }

    function approveInvoice(id) {
        if (!confirm('Are you sure you want to approve this publisher payment?')) return;

        fetch(`/admin/publisher-payment-approvals/${id}/approve`, {
            method: 'PATCH',
            headers: {
        ...
        .catch(() => alert('Failed to approve publisher payment.'));
    }

    function copyTable() {
        const table = document.getElementById('publisherPaymentApprovalsTable');
