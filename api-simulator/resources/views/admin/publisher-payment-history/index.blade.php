...
@section('title', 'Publisher Payment History')
...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Publisher Payment History</h1>
            <p class="text-sm text-gray-500 mt-1">Track monthly earnings by publisher for invoice generation.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
            <form method="GET" action="{{ route('admin.publisher-payment-history') }}" class="flex items-center gap-2 flex-1 flex-wrap">
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-44 bg-white rounded-xl border border-gray-200 shadow-lg py-1 z-50" style="display:none;">
                    ...
                    <a href="{{ route('admin.publisher-payment-history.export', request()->all()) }}" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">CSV</a>
        ...
        <div class="overflow-x-auto">
            <table id="publisherPaymentHistoryTable" class="w-full text-sm">
                <thead>
                    <tr class="border-t border-gray-100 bg-gray-50/50">
                        <th class="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-gray-400">Invoice Period</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($payments as $payment)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                <div class="font-medium text-gray-900">{{ $payment->publisher_name }}</div>
                        ...
                        <tr>
                            <td colspan="3" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-sm text-gray-500">No publisher payment history found.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
<script>
    // ─── COPY TABLE ──────────────────────────────────────
    function copyTable() {
        const table = document.getElementById('publisherPaymentHistoryTable');
