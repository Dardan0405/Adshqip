...
@section('title', 'Publisher Reports')
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Publisher Reports</h1>
            <p class="text-sm text-gray-500 mt-1">Monthly performance breakdown by publisher — impressions, clicks, conversions, earnings, CTR & ECPM.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
            <form method="GET" action="{{ route('admin.reports.publisher') }}" class="flex items-center gap-2 flex-1 flex-wrap">
                <div class="relative flex-1 max-w-xs">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by publisher name, email..."
                ...
                @if(request()->hasAny(['search','publisher_id','start_month','end_month']))
                    <a href="{{ route('admin.reports.publisher') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-500 hover:bg-gray-50 transition-colors">Clear</a>
                @endif
            </form>
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-44 bg-white rounded-xl border border-gray-200 shadow-lg py-1 z-50" style="display:none;">
                    ...
                    <a href="{{ route('admin.reports.publisher.export', request()->all()) }}" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">CSV</a>
        ...
        <div class="overflow-x-auto">
            <table id="publisherReportsTable" class="w-full text-sm">
                <thead>
                    ...
                    @empty
                        <tr><td colspan="8" class="px-4 py-12 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p class="text-sm text-gray-500">No publisher report data found.</p>
                            </div>
                        </td></tr>
                    @endforelse
                </tbody>
...
<script>
function copyTable() {
    const rows = document.getElementById('publisherReportsTable').querySelectorAll('tr');
    let text = '';
