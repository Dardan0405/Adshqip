...
@section('title', 'Advertiser Approvals')
...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Advertiser Approvals</h1>
            <p class="text-sm text-gray-500 mt-1">Review and approve or reject pending advertiser registrations.</p>
        </div>
        <button onclick="document.getElementById('addAdvertiserModal').classList.remove('hidden')"
    ...
    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
            <form method="GET" action="{{ route('admin.advertiser-approvals') }}" class="flex items-center gap-2 flex-1 flex-wrap">
                ...
                <a href="{{ route('admin.advertiser-approvals') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50">Reset</a>
            </form>
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-44 bg-white rounded-xl border border-gray-200 shadow-lg py-1 z-50" style="display:none;">
                    ...
                    <a href="{{ route('admin.advertiser-approvals.export', request()->all()) }}" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">CSV</a>
        ...
        <div class="overflow-x-auto">
            <table id="approvalTable" class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($pendingAdvertisers as $advertiser)
                        @php
                            $name = trim(($advertiser->userProfile->first_name ?? '') . ' ' . ($advertiser->userProfile->last_name ?? '')) ?: 'Unknown';
                        @endphp
                        <tr class="hover:bg-gray-50/50 transition-colors" id="row-{{ $advertiser->id }}">
                            ...
                            <td class="px-4 py-3 text-gray-600">{{ $advertiser->email }}</td>
                            <td class="px-4 py-3">
                                <div class="font-medium text-gray-900">{{ $advertiser->created_at?->format('M d, Y') }}</div>
                                <div class="text-xs text-gray-400">{{ $advertiser->created_at?->format('H:i') }}</div>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <div class="inline-flex items-center gap-2">
                                    <button onclick="approveAdvertiser({{ $advertiser->id }})"
                                    ...
                                    <button onclick="rejectAdvertiser({{ $advertiser->id }})"
                        ...
                        <tr>
                            <td colspan="5" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-sm text-gray-500">No pending advertiser approvals.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        @if($pendingAdvertisers->hasPages())
            <div class="px-4 py-3 border-t border-gray-100">
                {{ $pendingAdvertisers->links() }}
            </div>
        @endif
    </div>

    {{-- Add Advertiser Modal --}}
    <div id="addAdvertiserModal" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4">
            <div class="fixed inset-0 bg-gray-900/50 transition-opacity" onclick="document.getElementById('addAdvertiserModal').classList.add('hidden')"></div>
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg z-10">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <h3 class="text-lg font-bold text-gray-900">Add Advertiser</h3>
                    <button onclick="document.getElementById('addAdvertiserModal').classList.add('hidden')" class="p-1 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600">
                ...
                <form method="POST" action="{{ route('admin.advertiser-approvals.store') }}" class="p-6 space-y-4">
                    @csrf
                    ...
                    <div class="flex items-center justify-end gap-3 pt-2">
                        <button type="button" onclick="document.getElementById('addAdvertiserModal').classList.add('hidden')"
...
@push('scripts')
<script>
    function approveAdvertiser(id) {
        if (!confirm('Are you sure you want to approve this advertiser?')) return;

        fetch(`/admin/advertiser-approvals/${id}/approve`, {
            method: 'PATCH',
            headers: {
        ...
        .catch(() => alert('Failed to approve advertiser.'));
    }

    function rejectAdvertiser(id) {
        if (!confirm('Are you sure you want to reject this advertiser?')) return;

        fetch(`/admin/advertiser-approvals/${id}/reject`, {
            method: 'PATCH',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                'Accept': 'application/json',
        ...
        .catch(() => alert('Failed to reject advertiser.'));
    }
    ...
    @if($errors->any())
        document.getElementById('addAdvertiserModal').classList.remove('hidden');
    @endif
</script>
@endpush
