...
@section('title', 'Direct Campaign Approval')
...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Direct Campaign Approval</h1>
            <p class="text-sm text-gray-500 mt-1">Review direct campaign requests and approve or reject advertiser submissions.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
            <form method="GET" action="{{ route('admin.direct-campaign-request-approvals') }}" class="flex items-center gap-2 flex-1 flex-wrap">
                ...
                <a href="{{ route('admin.direct-campaign-request-approvals') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50">Reset</a>
            </form>
        </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($campaigns as $campaign)
                        @php
                            $campaignTypeLabel = $campaignTypes[$campaign->pricing_model] ?? strtoupper((string) $campaign->pricing_model);
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-4 py-3 text-right font-semibold text-blue-700">EUR {{ number_format((float) $campaign->approval_budget, 2) }}</td>
                            <td class="px-4 py-3 text-center">
                                <div class="inline-flex items-center gap-1.5 flex-wrap justify-center">
                                    @if($campaign->status === 'pending_review')
                                        <button onclick="rejectDirectCampaign({{ $campaign->id }})"
                                        ...
                                        <button onclick="approveDirectCampaign({{ $campaign->id }})"
                        ...
                        <tr>
                            <td colspan="8" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-sm text-gray-500">No direct campaign approval records found.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
@push('scripts')
<script>
    function approveDirectCampaign(id) {
        if (!confirm('Are you sure you want to approve this direct campaign?')) return;

        fetch(`/admin/direct-campaign-request-approvals/${id}/approve`, {
            method: 'PATCH',
            headers: {
        ...
        .catch(() => alert('Failed to approve direct campaign.'));
    }

    function rejectDirectCampaign(id) {
        if (!confirm('Are you sure you want to reject this direct campaign?')) return;

        fetch(`/admin/direct-campaign-request-approvals/${id}/reject`, {
            method: 'PATCH',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                'Accept': 'application/json',
        ...
        .catch(() => alert('Failed to reject direct campaign.'));
    }
</script>
@endpush
