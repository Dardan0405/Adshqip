...
@section('title', 'DSP Report')

@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">DSP Report</h1>
            <p class="text-sm text-gray-500 mt-1">Demand-Side Platform performance analytics and bidding metrics.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 mb-6">
        @php
            $statCards = [
                ...
                ['label' => 'Response Rate', 'value' => $summary['response_rate'] . '%', 'color' => 'text-violet-700', 'bg' => 'bg-violet-50', 'border' => 'border-violet-200', 'icon' => '<path d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>'],
                ['label' => 'Win Rate', 'value' => $summary['win_rate'] . '%', 'color' => 'text-amber-700', 'bg' => 'bg-amber-50', 'border' => 'border-amber-200', 'icon' => '<path d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>'],
    ...
    {{-- Filters Section --}}
    <form method="GET" action="{{ route('admin.reports.dsp') }}" class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
            <div class="flex items-center gap-2 flex-1 flex-wrap">
                ...
                <a href="{{ route('admin.reports.dsp') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50">Reset</a>
            </div>
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-44 bg-white rounded-xl border border-gray-200 shadow-lg py-1 z-50" style="display:none;">
                    ...
                    <a href="{{ route('admin.reports.dsp.export', request()->all()) }}" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">CSV</a>
    ...
    <div class="bg-white rounded-xl border border-gray-200">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 border-b border-gray-100">
            <div>
                <h2 class="text-sm font-semibold text-gray-900">Ad Exchange Performance</h2>
                <p class="text-xs text-gray-500 mt-1">{{ $dspData->count() }} exchanges with bidding data</p>
            </div>
        </div>

        <div class="overflow-x-auto">
            <table id="dspTable" class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/50">
                        ...
                        <th class="px-4 py-3 text-right text-[10px] font-semibold uppercase tracking-wider text-gray-400">Win Rate</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($dspData as $row)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold {{ $row->type === 'DSP' ? 'bg-blue-100 text-blue-700' : ($row->type === 'SSP' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-600') }}">
                                    {{ $row->type }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-3 text-right">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold {{ $row->win_rate >= 50 ? 'bg-emerald-100 text-emerald-700' : ($row->win_rate >= 25 ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600') }}">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <button type="button" onclick="viewStats({{ $row->id }})" class="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-blue-600" title="View Stats">
                        ...
                        <tr>
                            <td colspan="10" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-sm text-gray-500">No DSP data available for the selected period.</p>
                                    <a href="{{ route('admin.rtb') }}" class="text-sm text-brand-600 hover:underline">Add Ad Exchange</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
<script>
    function copyTable() {
        const table = document.getElementById('dspTable');
    ...
    function toggleStatus(id, currentStatus) {
        ...
        const message = currentStatus === 'active'
            ? 'Are you sure you want to pause this exchange?'
            : 'Are you sure you want to activate this exchange?';
