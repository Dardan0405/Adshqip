...
@section('title', 'Edit Direct Campaign')
...
<form method="POST" action="{{ route('admin.direct-campaigns.update', $campaign['id']) }}" enctype="multipart/form-data" id="directCampaignForm">
    @csrf
    @method('PUT')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <div class="flex items-center gap-2 text-sm text-gray-400 mb-1">
                <a href="{{ route('admin.direct-campaigns') }}" class="hover:text-gray-600 transition-colors">Direct Campaigns</a>
                ...
                <a href="{{ route('admin.direct-campaigns.show', $campaign['id']) }}" class="hover:text-gray-600 transition-colors">#{{ $campaign['id'] }}</a>
                <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none"><path d="M9 5l7 7-7 7" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>
                <span class="text-gray-600">Edit</span>
            </div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Direct Campaign</h1>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.direct-campaigns.show', $campaign['id']) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-200 bg-white text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">Cancel</a>
        ...
        <div class="p-6 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="name" required value="{{ old('name', $campaign['name']) }}" placeholder="e.g. Premium Direct — Albania Q4" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    ...
                    <select name="pricing_model" required class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white">
                        ...
                        @foreach($pricingModels as $key => $label)
                            <option value="{{ $key }}" {{ old('pricing_model', $campaign['pricing_model']) === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div>
                    ...
                    <select name="marketing_objective" required class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white">
                        ...
                        @foreach($marketingObjectives as $key => $label)
                            <option value="{{ $key }}" {{ old('marketing_objective', $campaign['marketing_objective']) === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Campaign Group Name</label>
                    <input type="text" name="campaign_group_name" value="{{ old('campaign_group_name', $campaign['campaign_group_name'] ?? '') }}" placeholder="e.g. Performance Q4" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="status" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white">
                        @foreach(['draft'=>'Draft','pending_review'=>'Pending Review','active'=>'Active','paused'=>'Paused','completed'=>'Completed','rejected'=>'Rejected','archived'=>'Archived'] as $k => $v)
                            <option value="{{ $k }}" {{ old('status', $campaign['status']) === $k ? 'selected' : '' }}>{{ $v }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Description</label>
                    <textarea name="description" rows="2" placeholder="Campaign description..." class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">{{ old('description', $campaign['description'] ?? '') }}</textarea>
                </div>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Notes</label>
                <textarea name="notes" rows="2" placeholder="Internal notes..." class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">{{ old('notes', $campaign['notes'] ?? '') }}</textarea>
            </div>
        </div>
    </div>
        ...
        <div class="p-6 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="destination_url" required value="{{ old('destination_url', $campaign['destination_url'] ?? '') }}" placeholder="https://example.com/landing" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Display URL</label>
                    <input type="text" name="display_url" value="{{ old('display_url', $campaign['display_url'] ?? '') }}" placeholder="example.com" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Tracking URL</label>
                    <input type="text" name="tracking_url" value="{{ old('tracking_url', $campaign['tracking_url'] ?? '') }}" placeholder="https://tracker.example.com/..." class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Click Tracking URL</label>
                    <input type="text" name="click_tracking_url" value="{{ old('click_tracking_url', $campaign['click_tracking_url'] ?? '') }}" placeholder="https://click.example.com/..." class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="p-6 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="datetime-local" name="start_date" value="{{ old('start_date', $campaign['start_date'] ?? '') }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">End Date</label>
                    <input type="datetime-local" name="end_date" value="{{ old('end_date', $campaign['end_date'] ?? '') }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="schedule_timezone" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white">
                        ...
                        @foreach($timezones as $tz => $label)
                            <option value="{{ $tz }}" {{ old('schedule_timezone', $campaign['schedule_timezone'] ?? 'UTC') === $tz ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="delivery_mode" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white">
                        <option value="standard" {{ old('delivery_mode', $campaign['delivery_mode'] ?? 'standard') === 'standard' ? 'selected' : '' }}>Standard</option>
                        <option value="accelerated" {{ old('delivery_mode', $campaign['delivery_mode'] ?? '') === 'accelerated' ? 'selected' : '' }}>Accelerated</option>
                    </select>
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Priority: <span id="priorityValue" class="text-brand-600">{{ $campaign['priority'] ?? 5 }}</span></label>
                    <input type="range" name="priority" min="1" max="10" value="{{ old('priority', $campaign['priority'] ?? 5) }}" class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-brand-600" oninput="document.getElementById('priorityValue').textContent = this.value">
                    <div class="flex justify-between text-[10px] text-gray-400 mt-1">
                        <span>1 (Low)</span><span>5 (Normal)</span><span>10 (High)</span>
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Weight: <span id="weightValue" class="text-brand-600">{{ $campaign['weight'] ?? 5 }}</span></label>
                    <input type="range" name="weight" min="1" max="10" value="{{ old('weight', $campaign['weight'] ?? 5) }}" class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-brand-600" oninput="document.getElementById('weightValue').textContent = this.value">
                    <div class="flex justify-between text-[10px] text-gray-400 mt-1">
                        <span>1 (Low)</span><span>5 (Normal)</span><span>10 (High)</span>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number" name="frequency_cap" min="1" value="{{ old('frequency_cap', $campaign['frequency_cap'] ?? '') }}" placeholder="e.g. 3" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    ...
                    <select name="frequency_cap_period" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white">
                        ...
                        @foreach(['hour'=>'Per Hour','day'=>'Per Day','week'=>'Per Week','month'=>'Per Month','lifetime'=>'Lifetime'] as $k => $v)
                            <option value="{{ $k }}" {{ old('frequency_cap_period', $campaign['frequency_cap_period'] ?? '') === $k ? 'selected' : '' }}>{{ $v }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            {{-- Dayparting --}}
            @php $hasDayparting = !empty($campaign['dayparting']); @endphp
            <div>
                ...
                <div class="flex items-center gap-4 mb-3">
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="weekly_mode" value="24_7" {{ !$hasDayparting ? 'checked' : '' }} class="text-brand-600 focus:ring-brand-500" onchange="document.getElementById('daypartingBlock').classList.add('hidden')">
                        <span class="text-sm text-gray-600">24/7 (Always On)</span>
                    </label>
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="weekly_mode" value="custom" {{ $hasDayparting ? 'checked' : '' }} class="text-brand-600 focus:ring-brand-500" onchange="document.getElementById('daypartingBlock').classList.remove('hidden')">
                        <span class="text-sm text-gray-600">Custom</span>
                    </label>
                </div>
                <div id="daypartingBlock" class="{{ $hasDayparting ? '' : 'hidden' }}">
                        ...
                        <table class="w-full text-[10px] table-fixed">
                            ...
                            <tbody>
                                @php
                                    ...
                                    $daypartData = $campaign['dayparting'] ?? [];
                                @endphp
                                @foreach($days as $day)
                                    @php $dayKey = strtolower($day); $dayHours = $daypartData[$dayKey] ?? []; @endphp
                                    <tr>
                                        ...
                                        @for($h = 0; $h < 24; $h++)
                                            <td class="py-1 px-0.5 text-center align-middle">
                                                <label class="inline-flex cursor-pointer items-center justify-center m-0" style="position: relative; display: inline-flex; width: 16px; height: 16px;">
                                                    <input type="checkbox" name="dayparting[{{ $dayKey }}][]" value="{{ $h }}" class="daypart-cell peer" data-day="{{ $dayKey }}" data-hour="{{ $h }}" {{ in_array($h, (array)$dayHours) ? 'checked' : '' }} style="position: absolute; opacity: 0; pointer-events: none; width: 1px; height: 1px; margin: 0;">
        ...
        <div class="p-6 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                    ...
                    <input type="number" name="bid_amount" required step="0.01" min="0" value="{{ old('bid_amount', $campaign['bid_amount'] ?? '') }}" placeholder="0.00" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Daily Budget</label>
                    <input type="number" name="daily_budget" step="0.01" min="0" value="{{ old('daily_budget', $campaign['daily_budget'] ?? '') }}" placeholder="0.00" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Total Budget</label>
                    <input type="number" name="total_budget" step="0.01" min="0" value="{{ old('total_budget', $campaign['total_budget'] ?? '') }}" placeholder="0.00" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                    ...
                    <select name="currency" required class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white">
                        <option value="EUR" {{ old('currency', $campaign['currency'] ?? 'EUR') === 'EUR' ? 'selected' : '' }}>EUR (&euro;)</option>
                        <option value="USD" {{ old('currency', $campaign['currency'] ?? '') === 'USD' ? 'selected' : '' }}>USD ($)</option>
                        <option value="GBP" {{ old('currency', $campaign['currency'] ?? '') === 'GBP' ? 'selected' : '' }}>GBP (&pound;)</option>
                        <option value="ALL" {{ old('currency', $campaign['currency'] ?? '') === 'ALL' ? 'selected' : '' }}>ALL (Lek)</option>
                    </select>
                </div>
                @if(isset($campaign['spend']))
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Amount Spent</label>
                    <div class="px-4 py-2.5 rounded-lg border border-gray-100 bg-gray-50 text-sm text-gray-600 font-medium tabular-nums">&euro;{{ number_format($campaign['spend'], 2) }}</div>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="p-6 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="headline" value="{{ old('headline', $campaign['headline'] ?? '') }}" placeholder="Ad headline text" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Call to Action</label>
                    <input type="text" name="call_to_action" value="{{ old('call_to_action', $campaign['call_to_action'] ?? '') }}" placeholder="e.g. Learn More, Shop Now" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Sponsored Label</label>
                    <input type="text" name="sponsored_label" value="{{ old('sponsored_label', $campaign['sponsored_label'] ?? '') }}" placeholder="e.g. Sponsored, Ad, Promoted" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Body Text</label>
                <textarea name="body_text" rows="3" placeholder="Ad body text..." class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">{{ old('body_text', $campaign['body_text'] ?? '') }}</textarea>
            </div>
            <div class="border-t border-gray-100 pt-5">
                ...
                <div id="dkiSection" class="{{ (!empty($campaign['headline_dki']) || !empty($campaign['body_text_dki'])) ? '' : 'hidden' }} space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-1">Headline DKI</label>
                            <input type="text" name="headline_dki" value="{{ old('headline_dki', $campaign['headline_dki'] ?? '') }}" placeholder="Headline with {keyword}" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-1">Headline DKI Default</label>
                            <input type="text" name="headline_dki_default" value="{{ old('headline_dki_default', $campaign['headline_dki_default'] ?? '') }}" placeholder="Default fallback headline" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-500 mb-1">Body Text DKI</label>
                        <textarea name="body_text_dki" rows="2" placeholder="Body text with {keyword}" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">{{ old('body_text_dki', $campaign['body_text_dki'] ?? '') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="p-6 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="brand_name" value="{{ old('brand_name', $campaign['brand_name'] ?? '') }}" placeholder="Your brand name" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Brand Tagline</label>
                    <input type="text" name="brand_tagline" value="{{ old('brand_tagline', $campaign['brand_tagline'] ?? '') }}" placeholder="Your brand tagline" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Brand Logo URL</label>
                <input type="text" name="brand_logo_url" value="{{ old('brand_logo_url', $campaign['brand_logo_url'] ?? '') }}" placeholder="https://example.com/logo.png" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Primary Color</label>
                    <div class="flex items-center gap-3">
                        <input type="color" name="brand_color_primary" value="{{ old('brand_color_primary', $campaign['brand_color_primary'] ?? '#6366f1') }}" class="w-10 h-10 rounded-lg border border-gray-200 cursor-pointer p-0.5">
                        <input type="text" id="brandColorPrimaryText" value="{{ old('brand_color_primary', $campaign['brand_color_primary'] ?? '#6366f1') }}" placeholder="#6366f1" class="flex-1 px-4 py-2.5 rounded-lg border border-gray-200 text-sm font-mono" oninput="document.querySelector('[name=brand_color_primary]').value = this.value">
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Secondary Color</label>
                    <div class="flex items-center gap-3">
                        <input type="color" name="brand_color_secondary" value="{{ old('brand_color_secondary', $campaign['brand_color_secondary'] ?? '#8b5cf6') }}" class="w-10 h-10 rounded-lg border border-gray-200 cursor-pointer p-0.5">
                        <input type="text" id="brandColorSecondaryText" value="{{ old('brand_color_secondary', $campaign['brand_color_secondary'] ?? '#8b5cf6') }}" placeholder="#8b5cf6" class="flex-1 px-4 py-2.5 rounded-lg border border-gray-200 text-sm font-mono" oninput="document.querySelector('[name=brand_color_secondary]').value = this.value">
                    </div>
                </div>
            </div>
        </div>
    ...
    {{-- ═══════════════════════════════════════════════════════ --}}
    @php $ctwEnabled = old('ctw_enabled', $campaign['ctw_enabled'] ?? false); @endphp
    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="px-6 py-4 border-b border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    ...
                    <input type="checkbox" name="ctw_enabled" value="1" id="ctwToggle" class="sr-only peer" {{ $ctwEnabled ? 'checked' : '' }} onchange="document.getElementById('ctwSection').classList.toggle('hidden', !this.checked)">
        ...
        <div id="ctwSection" class="{{ $ctwEnabled ? '' : 'hidden' }} p-6 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">CTW Thumbnail URL</label>
                    <input type="text" name="ctw_thumbnail_url" value="{{ old('ctw_thumbnail_url', $campaign['ctw_thumbnail_url'] ?? '') }}" placeholder="https://example.com/thumbnail.jpg" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Min Watch Seconds</label>
                    <input type="number" name="ctw_min_watch_seconds" min="1" value="{{ old('ctw_min_watch_seconds', $campaign['ctw_min_watch_seconds'] ?? '') }}" placeholder="e.g. 5" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Skip After Seconds</label>
                    <input type="number" name="ctw_skip_after_seconds" min="1" value="{{ old('ctw_skip_after_seconds', $campaign['ctw_skip_after_seconds'] ?? '') }}" placeholder="e.g. 15" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div class="flex items-center gap-8">
                <div class="flex items-center gap-3">
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <input type="checkbox" name="ctw_autoplay" value="1" class="sr-only peer" {{ old('ctw_autoplay', $campaign['ctw_autoplay'] ?? false) ? 'checked' : '' }}>
                        <div class="w-10 h-5 bg-gray-200 peer-focus:ring-2 peer-focus:ring-brand-500/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-brand-600"></div>
                    </label>
                ...
                <div class="flex items-center gap-3">
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <input type="checkbox" name="ctw_muted_autoplay" value="1" class="sr-only peer" {{ old('ctw_muted_autoplay', $campaign['ctw_muted_autoplay'] ?? false) ? 'checked' : '' }}>
                        <div class="w-10 h-5 bg-gray-200 peer-focus:ring-2 peer-focus:ring-brand-500/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-brand-600"></div>
                    </label>
    ...
    {{-- ═══════════════════════════════════════════════════════ --}}
    @php $endCardEnabled = old('end_card_enabled', $campaign['end_card_enabled'] ?? false); @endphp
    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="px-6 py-4 border-b border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    ...
                    <input type="checkbox" name="end_card_enabled" value="1" id="endCardToggle" class="sr-only peer" {{ $endCardEnabled ? 'checked' : '' }} onchange="document.getElementById('endCardSection').classList.toggle('hidden', !this.checked)">
                    <div class="w-10 h-5 bg-gray-200 peer-focus:ring-2 peer-focus:ring-brand-500/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-brand-600"></div>
                </label>
            </div>
        </div>
        <div id="endCardSection" class="{{ $endCardEnabled ? '' : 'hidden' }} p-6 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="end_card_type" id="endCardType" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white" onchange="document.getElementById('endCardHtmlRow').classList.toggle('hidden', this.value !== 'html')">
                        @foreach(['static_image'=>'Static Image','html'=>'HTML','cta_button'=>'CTA Button','product_feed'=>'Product Feed','custom'=>'Custom'] as $k => $v)
                            <option value="{{ $k }}" {{ old('end_card_type', $campaign['end_card_type'] ?? '') === $k ? 'selected' : '' }}>{{ $v }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Display Seconds</label>
                    <input type="number" name="end_card_display_seconds" min="1" value="{{ old('end_card_display_seconds', $campaign['end_card_display_seconds'] ?? '') }}" placeholder="e.g. 10" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">End Card Headline</label>
                    <input type="text" name="end_card_headline" value="{{ old('end_card_headline', $campaign['end_card_headline'] ?? '') }}" placeholder="End card headline" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">CTA Text</label>
                    <input type="text" name="end_card_cta_text" value="{{ old('end_card_cta_text', $campaign['end_card_cta_text'] ?? '') }}" placeholder="e.g. Visit Now" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">End Card Body</label>
                <textarea name="end_card_body" rows="2" placeholder="End card body text..." class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">{{ old('end_card_body', $campaign['end_card_body'] ?? '') }}</textarea>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Image URL</label>
                    <input type="text" name="end_card_image_url" value="{{ old('end_card_image_url', $campaign['end_card_image_url'] ?? '') }}" placeholder="https://example.com/endcard.jpg" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">CTA URL</label>
                    <input type="text" name="end_card_cta_url" value="{{ old('end_card_cta_url', $campaign['end_card_cta_url'] ?? '') }}" placeholder="https://example.com/action" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="color" name="end_card_cta_color" value="{{ old('end_card_cta_color', $campaign['end_card_cta_color'] ?? '#6366f1') }}" class="w-10 h-10 rounded-lg border border-gray-200 cursor-pointer p-0.5">
                        <input type="text" value="{{ old('end_card_cta_color', $campaign['end_card_cta_color'] ?? '#6366f1') }}" placeholder="#6366f1" class="flex-1 px-4 py-2.5 rounded-lg border border-gray-200 text-sm font-mono" oninput="document.querySelector('[name=end_card_cta_color]').value = this.value">
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Logo URL</label>
                    <input type="text" name="end_card_logo_url" value="{{ old('end_card_logo_url', $campaign['end_card_logo_url'] ?? '') }}" placeholder="https://example.com/logo.png" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div id="endCardHtmlRow" class="{{ old('end_card_type', $campaign['end_card_type'] ?? '') === 'html' ? '' : 'hidden' }}">
                <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">End Card HTML</label>
                <textarea name="end_card_html" rows="4" placeholder="Custom HTML code..." class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 font-mono">{{ old('end_card_html', $campaign['end_card_html'] ?? '') }}</textarea>
            </div>
        </div>
    </div>
    ...
    {{-- ═══════════════════════════════════════════════════════ --}}
    @php $clipEnabled = old('clip_enabled', $campaign['clip_enabled'] ?? false); @endphp
    @php $swipeUpEnabled = old('clip_swipe_up_enabled', $campaign['clip_swipe_up_enabled'] ?? false); @endphp
    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="px-6 py-4 border-b border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    ...
                    <input type="checkbox" name="clip_enabled" value="1" id="clipToggle" class="sr-only peer" {{ $clipEnabled ? 'checked' : '' }} onchange="document.getElementById('clipSection').classList.toggle('hidden', !this.checked)">
                    <div class="w-10 h-5 bg-gray-200 peer-focus:ring-2 peer-focus:ring-brand-500/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-brand-600"></div>
                </label>
            </div>
        </div>
        <div id="clipSection" class="{{ $clipEnabled ? '' : 'hidden' }} p-6 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Clip Video URL</label>
                    <input type="text" name="clip_video_url" value="{{ old('clip_video_url', $campaign['clip_video_url'] ?? '') }}" placeholder="https://example.com/clip.mp4" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Clip Thumbnail URL</label>
                    <input type="text" name="clip_thumbnail_url" value="{{ old('clip_thumbnail_url', $campaign['clip_thumbnail_url'] ?? '') }}" placeholder="https://example.com/thumb.jpg" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Duration (seconds)</label>
                    <input type="number" name="clip_duration_seconds" min="1" value="{{ old('clip_duration_seconds', $campaign['clip_duration_seconds'] ?? '') }}" placeholder="e.g. 15" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                </div>
                <div>
                    ...
                    <select name="clip_aspect_ratio" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white">
                        <option value="9:16" {{ old('clip_aspect_ratio', $campaign['clip_aspect_ratio'] ?? '') === '9:16' ? 'selected' : '' }}>9:16 (Vertical)</option>
                        <option value="4:5" {{ old('clip_aspect_ratio', $campaign['clip_aspect_ratio'] ?? '') === '4:5' ? 'selected' : '' }}>4:5 (Portrait)</option>
                        <option value="1:1" {{ old('clip_aspect_ratio', $campaign['clip_aspect_ratio'] ?? '') === '1:1' ? 'selected' : '' }}>1:1 (Square)</option>
                    </select>
                </div>
            </div>
            <div class="flex items-center gap-8">
                <div>
                    ...
                    <select name="clip_sound_default" class="px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white">
                        <option value="off" {{ old('clip_sound_default', $campaign['clip_sound_default'] ?? '') === 'off' ? 'selected' : '' }}>Off</option>
                        <option value="on" {{ old('clip_sound_default', $campaign['clip_sound_default'] ?? '') === 'on' ? 'selected' : '' }}>On</option>
                    </select>
                </div>
                <div class="flex items-center gap-3 pt-5">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="hidden" name="clip_autoplay" value="0">
                        <input type="checkbox" name="clip_autoplay" value="1" class="sr-only peer" {{ old('clip_autoplay', $campaign['clip_autoplay'] ?? false) ? 'checked' : '' }}>
                        <div class="w-10 h-5 bg-gray-200 peer-focus:ring-2 peer-focus:ring-brand-500/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-brand-600"></div>
                    </label>
                    <span class="text-sm font-medium text-gray-700">Autoplay</span>
                </div>
                <div class="flex items-center gap-3 pt-5">
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <input type="checkbox" name="clip_loop" value="1" class="sr-only peer" {{ old('clip_loop', $campaign['clip_loop'] ?? false) ? 'checked' : '' }}>
                        <div class="w-10 h-5 bg-gray-200 peer-focus:ring-2 peer-focus:ring-brand-500/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-brand-600"></div>
                    </label>
            ...
            <div>
                ...
                <textarea name="clip_caption" rows="2" placeholder="Clip caption text..." class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">{{ old('clip_caption', $campaign['clip_caption'] ?? '') }}</textarea>
            </div>
            <div class="border-t border-gray-100 pt-5">
                <div class="flex items-center gap-3 mb-4">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="hidden" name="clip_swipe_up_enabled" value="0">
                        <input type="checkbox" name="clip_swipe_up_enabled" value="1" class="sr-only peer" {{ $swipeUpEnabled ? 'checked' : '' }} onchange="document.getElementById('swipeUpFields').classList.toggle('hidden', !this.checked)">
                        <div class="w-10 h-5 bg-gray-200 peer-focus:ring-2 peer-focus:ring-brand-500/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-brand-600"></div>
                    </label>
                    <span class="text-sm font-medium text-gray-700">Enable Swipe Up</span>
                </div>
                <div id="swipeUpFields" class="{{ $swipeUpEnabled ? '' : 'hidden' }} grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Swipe Up Text</label>
                        <input type="text" name="clip_swipe_up_text" value="{{ old('clip_swipe_up_text', $campaign['clip_swipe_up_text'] ?? '') }}" placeholder="Swipe up to learn more" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Swipe Up URL</label>
                        <input type="text" name="clip_swipe_up_url" value="{{ old('clip_swipe_up_url', $campaign['clip_swipe_up_url'] ?? '') }}" placeholder="https://example.com/landing" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                    </div>
                </div>
            </div>
        </div>
    ...
    {{-- ═══════════════════════════════════════════════════════ --}}
    @php $targeting = $campaign['targeting_by_type'] ?? []; @endphp
    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="px-6 py-4 border-b border-gray-100">
            <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <select id="geoSelect" multiple class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white min-h-[120px]">
                    @foreach($countries as $country)
                        <option value="{{ $country['code'] }}" {{ in_array($country['code'], $targeting['geo_country'] ?? []) ? 'selected' : '' }}>{{ $country['name'] }} ({{ $country['code'] }})</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-4">
                    @foreach(['desktop'=>'Desktop', 'mobile'=>'Mobile', 'tablet'=>'Tablet'] as $val => $label)
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" class="device-cb rounded border-gray-300 text-brand-600 focus:ring-brand-500" value="{{ $val }}" {{ in_array($val, $targeting['device'] ?? []) ? 'checked' : '' }}>
                    ...
                    @foreach($operatingSystems as $family => $versions)
                        <optgroup label="{{ $family }}">
                            @foreach($versions as $version)
                                <option value="{{ $version }}" {{ in_array($version, $targeting['os'] ?? []) ? 'selected' : '' }}>{{ $version }}</option>
                            @endforeach
                        </optgroup>
                    @endforeach
                </select>
                ...
                <select id="browserSelect" multiple class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white min-h-[120px]">
                    @foreach($browsers as $browser => $versions)
                        <optgroup label="{{ $browser }}">
                            @foreach($versions as $version)
                                <option value="{{ $version }}" {{ in_array($version, $targeting['browser'] ?? []) ? 'selected' : '' }}>{{ $version }}</option>
                            @endforeach
                        </optgroup>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select id="languageSelect" multiple class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white min-h-[120px]">
                    @foreach($languages as $code => $name)
                        <option value="{{ $code }}" {{ in_array($code, $targeting['language'] ?? []) ? 'selected' : '' }}>{{ $name }} ({{ $code }})</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach($connectionTypes as $ct)
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" class="connection-type-cb rounded border-gray-300 text-brand-600 focus:ring-brand-500" value="{{ $ct }}" {{ in_array($ct, $targeting['connection_type'] ?? []) ? 'checked' : '' }}>
                    ...
                    @foreach($mobileCarriers as $countryCode => $carriers)
                        <optgroup label="{{ $countryCode }}">
                            @foreach($carriers as $carrier)
                                <option value="{{ $carrier }}" {{ in_array($carrier, $targeting['carrier'] ?? []) ? 'selected' : '' }}>{{ $carrier }}</option>
                            @endforeach
                        </optgroup>
                    @endforeach
                </select>
    ...
    <div class="flex items-center justify-end gap-3 mb-10">
        <a href="{{ route('admin.direct-campaigns.show', $campaign['id']) }}" class="px-5 py-2.5 rounded-lg border border-gray-200 bg-white text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">Cancel</a>
        <button type="submit" class="px-6 py-2.5 rounded-lg bg-brand-600 text-sm font-semibold text-white hover:bg-brand-700 shadow-sm transition-colors">Update Campaign</button>
    </div>
</form>
