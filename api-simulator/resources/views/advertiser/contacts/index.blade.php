...
<div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Keep advertiser-side contacts in one place and quickly reach platform support teams.</p>
        </div>
        <a href="{{ route('advertiser.contacts.export', request()->query()) }}" class="inline-flex w-fit rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50">Export CSV</a>
    </div>
    ...
    <div class="grid gap-6 xl:grid-cols-3">
        <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <form method="POST" action="{{ route('advertiser.contacts.store') }}" class="mt-4 space-y-4">
                @csrf
                <div>
        ...
        <div class="space-y-4 xl:col-span-2">
            <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                <form method="GET" action="{{ route('advertiser.contacts') }}" class="grid gap-3 lg:grid-cols-5">
                    ...
                    <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                        <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <form method="POST" action="{{ route('advertiser.contacts.touch', $contact) }}">
                                    @csrf
                                    @method('PATCH')
                                ...
                                <form method="POST" action="{{ route('advertiser.contacts.destroy', $contact) }}" onsubmit="return confirm('Delete this contact?');">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <details class="mt-4 rounded-lg border border-gray-100 bg-gray-50 p-4">
                            ...
                            <form method="POST" action="{{ route('advertiser.contacts.update', $contact) }}" class="mt-4 grid gap-3 lg:grid-cols-2">
                                @csrf
                                @method('PUT')
                                <div>
                                    <label class="text-sm font-medium text-gray-700">Name</label>
