...
<div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
            <a href="{{ route('advertiser.support-tickets') }}" class="text-sm font-semibold text-brand-700 hover:text-brand-800">Back to Support Tickets</a>
        ...
        @if(! in_array($ticket->status, ['resolved', 'closed'], true))
            <form method="POST" action="{{ route('advertiser.support-tickets.close', $ticket) }}">
                @csrf
                @method('PATCH')
    ...
    @if(! in_array($ticket->status, ['resolved', 'closed'], true))
        <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <form method="POST" action="{{ route('advertiser.support-tickets.reply', $ticket) }}" class="mt-4 space-y-4">
                @csrf
