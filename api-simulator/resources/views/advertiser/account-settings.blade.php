...
@section('content')
    ...
    <div class="space-y-6">
        ...
        <form method="POST" action="{{ route('advertiser.account-settings.update') }}" enctype="multipart/form-data" class="grid grid-cols-1 gap-6 xl:grid-cols-3">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6 xl:col-span-2">
                <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="mb-6 flex items-center justify-between gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-slate-500">This information is used in your account and admin identity.</p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        <div class="md:col-span-2">
                            ...
                            <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-rose-400 focus:outline-none focus:ring-4 focus:ring-rose-100">
                        ...
                        <div>
                            ...
                            <input type="email" name="alternative_email" value="{{ old('alternative_email', $profile?->alternative_email) }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-rose-400 focus:outline-none focus:ring-4 focus:ring-rose-100" placeholder="backup@example.com">
                ...
                <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="mb-6">
                        ...
                        <p class="mt-1 text-sm text-slate-500">Change your password and manage the full 2-factor authentication login flow.</p>
                    </div>

                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        <div class="md:col-span-2">
                            <label class="inline-flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                                <input type="checkbox" name="two_factor_enabled" value="1" {{ old('two_factor_enabled', $user->two_factor_enabled) ? 'checked' : '' }} class="mt-1 h-4 w-4 rounded border-slate-300 text-rose-600 focus:ring-rose-500">
                                <span>
                        ...
                        <div>
                            ...
                            <input type="password" name="current_password" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-rose-400 focus:outline-none focus:ring-4 focus:ring-rose-100">
                        ...
                        <div>
                            ...
                            <input type="password" name="new_password" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-rose-400 focus:outline-none focus:ring-4 focus:ring-rose-100">
                            @error('new_password') <p class="mt-1.5 text-xs text-rose-500">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">Confirmed Password</label>
                            <input type="password" name="new_password_confirmation" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-rose-400 focus:outline-none focus:ring-4 focus:ring-rose-100">
                        </div>
                    </div>
                    ...
                    <div class="mt-6 rounded-3xl border border-slate-200 bg-slate-50 p-5">
                        ...
                        @if($twoFactorSecret)
                            <div class="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
                                <div class="rounded-2xl bg-white p-4 shadow-sm">
                                    ...
                                    <p class="mt-3 text-xs leading-5 text-slate-500">Enter this exact key in your authenticator app if you are setting it up manually.</p>
                                </div>
                                <div class="rounded-2xl bg-white p-4 shadow-sm">
                                    ...
                                    <p class="mt-3 text-xs leading-5 text-slate-500">Advanced apps can import this URI directly. It contains your account label and issuer name.</p>
                                </div>
                            </div>
                        @else
                        ...
                        @if(!empty($backupCodes))
                            <div class="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4">
                                ...
                                <p class="mt-1 text-xs leading-5 text-amber-800">Store these in a safe place. Each backup code works once, and this is the only time the plain codes are shown.</p>
                        ...
                        @elseif($user->two_factor_enabled)
                            <div class="mt-4 rounded-2xl border border-slate-200 bg-white px-4 py-4 text-sm text-slate-600">
                                <span class="font-semibold text-slate-900">{{ $backupCodeCount }}</span> backup codes are currently available. Check "Regenerate backup codes" and save if you want a fresh set.
                            </div>
                        @endif
                    </div>
                </section>
            ...
            <div class="space-y-6">
                <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="mb-5">
                        ...
                        <p class="mt-1 text-sm text-slate-500">Select which image will be used for your profile picture.</p>
                    </div>
                    <div class="rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-5 text-center">
                        @if($profile?->avatar_url)
                            <img src="{{ $profile->avatar_url }}" alt="Current profile image" class="mx-auto h-28 w-28 rounded-3xl object-cover shadow-sm">
                        @else
                            <div class="mx-auto flex h-28 w-28 items-center justify-center rounded-3xl bg-gradient-to-br from-rose-500 to-orange-400 text-3xl font-bold text-white shadow-sm">
                                {{ $initials }}
                            </div>
                        @endif
                        <div class="mt-4">
                            <input type="file" name="avatar" accept=".jpg,.jpeg,.png,.webp" class="block w-full text-sm text-slate-500 file:mr-4 file:rounded-full file:border-0 file:bg-slate-900 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white hover:file:bg-slate-800">
                            <p class="mt-2 text-xs text-slate-400">PNG, JPG, or WEBP up to 3MB.</p>
                ...
                <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="mb-5">
                        ...
                        <p class="mt-1 text-sm text-slate-500">Set how monetary values should be shown in the dashboard.</p>
                    </div>
                    <div>
                    ...
                    <div class="mt-5 rounded-2xl bg-slate-50 p-4">
                        ...
                        <div class="mt-3 flex items-center justify-between">
                            ...
                            <span class="text-sm font-semibold text-slate-900">{{ ucfirst($user->role ?? 'admin') }}</span>
                        </div>
                    </div>
                </section>
                ...
                <a href="{{ route('advertiser.company-information') }}" class="inline-flex w-full items-center justify-center rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                    Company Information
                </a>
            </div>
        </form>
