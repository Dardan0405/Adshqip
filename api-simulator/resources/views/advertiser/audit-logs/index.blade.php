...
@section('content')
    ...
    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
            <form method="GET" action="{{ route('advertiser.audit-logs') }}" class="flex items-center gap-2 flex-1 flex-wrap">
                ...
                @if(request()->hasAny(['search', 'date', 'browser', 'os']))
                    <a href="{{ route('advertiser.audit-logs') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-500 hover:bg-gray-50 transition-colors">Clear</a>
                @endif
            </form>
        </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($logs as $log)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                <form method="POST" action="{{ route('advertiser.audit-logs.destroy', $log) }}" onsubmit="return confirm('Delete this audit log entry?');">
                                    @csrf
                                    @method('DELETE')
