...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Daily performance across your campaigns.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        ...
        <div class="rounded-xl border border-purple-200 bg-purple-50 p-4">
            <p class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">Conversions</p>
            <p class="mt-2 text-2xl font-bold text-purple-700">{{ number_format($summary->total_conversions) }}</p>
        </div>
        <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
            <p class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">Spend</p>
            <p class="mt-2 text-2xl font-bold text-emerald-700">&euro;{{ number_format($summary->total_spend, 2) }}</p>
        </div>
    </div>

    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
            <form method="GET" action="{{ route('advertiser.reports.overview') }}" class="flex items-center gap-2 flex-1 flex-wrap">
                ...
                @if(request()->hasAny(['start_date', 'end_date']))
                    <a href="{{ route('advertiser.reports.overview') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-500 hover:bg-gray-50 transition-colors">Clear</a>
                @endif
            </form>
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    ...
                    <button type="button" onclick="copyOverviewReportTable()" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Copy</button>
                    <a href="{{ route('advertiser.reports.overview.export', request()->all()) }}" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">CSV</a>
        ...
        <div class="overflow-x-auto">
            <table id="overviewReportsTable" class="w-full text-sm">
                <thead>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($reports as $row)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-4 py-3 text-right whitespace-nowrap font-medium text-purple-700">{{ number_format($row->total_conversions) }}</td>
                            <td class="px-4 py-3 text-right whitespace-nowrap font-semibold text-emerald-700">&euro;{{ number_format($row->total_spend, 2) }}</td>
                            <td class="px-4 py-3 text-right whitespace-nowrap font-medium text-gray-700">{{ number_format($row->ctr, 2) }}%</td>
                            <td class="px-4 py-3 text-right whitespace-nowrap font-medium text-gray-700">&euro;{{ number_format($row->ecpm, 2) }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-sm text-gray-500">No overview report data found.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
@push('scripts')
<script>
    function copyOverviewReportTable() {
        const table = document.getElementById('overviewReportsTable');
