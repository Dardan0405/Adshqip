...
@section('content')
    ...
    <div class="space-y-6">
        <div class="relative overflow-hidden rounded-[28px] border border-slate-200 bg-gradient-to-br from-slate-950 via-slate-900 to-brand-900 px-6 py-7 text-white shadow-sm">
            ...
            <div class="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="max-w-2xl">
                    ...
                    <p class="mt-3 max-w-xl text-sm leading-6 text-slate-200/85">
                        Configure trusted-device checks, delivery channels, and recovery options for this admin account.
                    </p>
                </div>
        ...
        <form method="POST" action="{{ route('advertiser.two-factor-authentication.update') }}" class="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.4fr)_360px]">
            @csrf
            @method('PUT')
            ...
            <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    @foreach($verificationOptions as $value => $label)
                        <label class="flex items-start gap-3 rounded-2xl border border-slate-200 px-4 py-4">
                            <input type="checkbox" name="verification_options[]" value="{{ $value }}" {{ in_array($value, old('verification_options', $enabledVerificationOptions), true) ? 'checked' : '' }} class="mt-1 h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500">
                            <span>
                ...
                <div class="mt-8 border-t border-slate-200 pt-6">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <label class="flex items-start gap-3 rounded-2xl border border-slate-200 px-4 py-4">
                            <input type="checkbox" name="token_types[]" value="sms" {{ in_array('sms', old('token_types', $enabledTokenTypes), true) ? 'checked' : '' }} class="mt-1 h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500">
                            <span class="flex-1">
                                ...
                                <input type="text" name="two_factor_phone" value="{{ old('two_factor_phone', $user->two_factor_phone) }}" class="mt-3 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-brand-400 focus:outline-none focus:ring-4 focus:ring-brand-100" placeholder="+355 69 000 0000">
                                @error('two_factor_phone') <p class="mt-1.5 text-xs text-rose-500">{{ $message }}</p> @enderror
                            </span>
                        </label>

                        <label class="flex items-start gap-3 rounded-2xl border border-slate-200 px-4 py-4">
                            <input type="checkbox" name="token_types[]" value="email_otp" {{ in_array('email_otp', old('token_types', $enabledTokenTypes), true) ? 'checked' : '' }} class="mt-1 h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500">
                            <span class="flex-1">
                                ...
                                <input type="email" name="two_factor_email" value="{{ old('two_factor_email', $user->two_factor_email) }}" class="mt-3 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-brand-400 focus:outline-none focus:ring-4 focus:ring-brand-100" placeholder="security@example.com">
                                @error('two_factor_email') <p class="mt-1.5 text-xs text-rose-500">{{ $message }}</p> @enderror
                            </span>
                        </label>

                        <label class="flex items-start gap-3 rounded-2xl border border-slate-200 px-4 py-4">
                            <input type="checkbox" name="token_types[]" value="backup_code" {{ in_array('backup_code', old('token_types', $enabledTokenTypes), true) ? 'checked' : '' }} class="mt-1 h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500">
                            <span class="flex-1">
                                ...
                                <input type="text" name="two_factor_backup_question" value="{{ old('two_factor_backup_question', $user->two_factor_backup_question) }}" class="mt-3 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-brand-400 focus:outline-none focus:ring-4 focus:ring-brand-100" placeholder="Question">
                                @error('two_factor_backup_question') <p class="mt-1.5 text-xs text-rose-500">{{ $message }}</p> @enderror
                                <input type="text" name="two_factor_backup_answer" value="" class="mt-3 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-brand-400 focus:outline-none focus:ring-4 focus:ring-brand-100" placeholder="Answer">
            ...
            <div class="space-y-6">
                <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="mb-5">
                        ...
                        <p class="mt-1 text-sm text-slate-500">A quick summary of the current trusted-device state and recovery capacity.</p>
                    </div>
