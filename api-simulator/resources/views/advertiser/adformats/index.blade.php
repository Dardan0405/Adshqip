...
<div class="bg-white rounded-xl border border-gray-200">
            ...
            <div class="flex items-center gap-2">
                <div class="relative" x-data="{ open: false }">
                    ...
                    <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-44 bg-white rounded-xl border border-gray-200 shadow-lg py-1 z-50" style="display: none;">
                        <a href="{{ route('advertiser.adformats.export', ['format' => 'csv']) }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
                        ...
                        <a href="{{ route('advertiser.adformats.export', ['format' => 'excel']) }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
                        ...
                        <a href="{{ route('advertiser.adformats.export', ['format' => 'pdf']) }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
    ...
    <div class="px-6 py-3 border-b border-gray-100 bg-gray-50/50">
        <form method="GET" action="{{ route('advertiser.adformats') }}" class="flex flex-col md:flex-row md:items-center gap-3">
            {{-- Search --}}
            ...
            @if($search !== '' || $typeFilter !== 'all')
                <a href="{{ route('advertiser.adformats') }}" class="px-3 py-2 rounded-lg border border-gray-200 text-sm text-gray-500 hover:bg-gray-50 transition-colors">Clear</a>
            @endif
        </form>
    </div>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1" x-data>
                                {{-- Edit --}}
                                <a href="{{ route('advertiser.adformats.edit', $ad['id']) }}" class="p-1.5 rounded-lg hover:bg-amber-50 text-gray-400 hover:text-amber-600 transition-colors" title="Edit">
                                ...
                                {{-- Demo --}}
                                <a href="{{ route('advertiser.adformats.demo', $ad['id']) }}" class="p-1.5 rounded-lg hover:bg-indigo-50 text-gray-400 hover:text-indigo-600 transition-colors" title="Demo">
                                ...
                                {{-- Reports --}}
                                <a href="{{ route('advertiser.adformats.reports', $ad['id']) }}" class="p-1.5 rounded-lg hover:bg-emerald-50 text-gray-400 hover:text-emerald-600 transition-colors" title="Reports">
        ...
        <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
            ...
            <div class="flex items-center gap-1">
                @if($pagination['current_page'] > 1)
                    <a href="{{ route('advertiser.adformats', array_merge(request()->query(), ['page' => $pagination['current_page'] - 1])) }}" class="px-3 py-1.5 rounded-lg border border-gray-200 text-xs text-gray-600 hover:bg-gray-50 transition-colors">Prev</a>
                @endif

                @for($p = max(1, $pagination['current_page'] - 2); $p <= min($pagination['total_pages'], $pagination['current_page'] + 2); $p++)
                    <a href="{{ route('advertiser.adformats', array_merge(request()->query(), ['page' => $p])) }}" class="px-3 py-1.5 rounded-lg text-xs {{ $p === $pagination['current_page'] ? 'bg-brand-600 text-white' : 'border border-gray-200 text-gray-600 hover:bg-gray-50' }} transition-colors">{{ $p }}</a>
                @endfor

                @if($pagination['current_page'] < $pagination['total_pages'])
                    <a href="{{ route('advertiser.adformats', array_merge(request()->query(), ['page' => $pagination['current_page'] + 1])) }}" class="px-3 py-1.5 rounded-lg border border-gray-200 text-xs text-gray-600 hover:bg-gray-50 transition-colors">Next</a>
                @endif
            </div>
        </div>
    @endif
