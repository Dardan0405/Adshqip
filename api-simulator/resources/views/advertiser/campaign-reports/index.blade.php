...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Campaign Reports</h1>
            <p class="text-sm text-gray-500 mt-1">Monthly performance breakdown by campaign — impressions, clicks, conversions, spend, CTR & ECPM.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
            <form method="GET" action="{{ route('advertiser.reports.campaign') }}" class="flex items-center gap-2 flex-1 flex-wrap">
                {{-- Search --}}
                <div class="relative flex-1 max-w-xs">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by campaign, advertiser..."
                ...
                {{-- Date range --}}
                <input type="month" name="start_month" value="{{ request('start_month') }}" placeholder="Start Month"
                       class="px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20">
                <input type="month" name="end_month" value="{{ request('end_month') }}" placeholder="End Month"
                       class="px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20">
                ...
                @if(request()->hasAny(['search', 'campaign_id', 'start_month', 'end_month']))
                    <a href="{{ route('advertiser.reports.campaign') }}"
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    ...
                    <a href="{{ route('advertiser.reports.campaign.export', request()->all()) }}"
        ...
        <div class="overflow-x-auto">
            <table id="campaignReportsTable" class="w-full text-sm">
                <thead>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($reports as $row)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-4 py-3">
                                <div class="font-medium text-gray-900">{{ $row->campaign_name }}</div>
                                <div class="mt-0.5">
                                    @php
                                        $statusClasses = match($row->campaign_status ?? '') {
                        ...
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-sm text-gray-500">No campaign report data found.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
<script>
    function copyTable() {
        const table = document.getElementById('campaignReportsTable');
