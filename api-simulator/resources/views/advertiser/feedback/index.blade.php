...
<div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <a href="{{ route('advertiser.feedback.export', request()->query()) }}" class="inline-flex w-fit rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50">Export CSV</a>
    </div>
    ...
    <div class="grid gap-6 xl:grid-cols-3">
        <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <form method="POST" action="{{ route('advertiser.feedback.store') }}" class="mt-4 space-y-4">
                @csrf
        ...
        <div class="space-y-4 xl:col-span-2">
            <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                <form method="GET" action="{{ route('advertiser.feedback') }}" class="grid gap-3 lg:grid-cols-4">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($feedback as $item)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-5 py-4">
                                        <a href="{{ route('advertiser.feedback.show', $item) }}" class="font-semibold text-brand-700 hover:text-brand-800">#{{ $item->id }} {{ $item->subject }}</a>
