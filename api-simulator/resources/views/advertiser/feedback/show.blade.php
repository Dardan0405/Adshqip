...
<div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
            <a href="{{ route('advertiser.feedback') }}" class="text-sm font-semibold text-brand-700 hover:text-brand-800">Back to Feedback</a>
        ...
        @if($item->status !== 'closed')
            <form method="POST" action="{{ route('advertiser.feedback.close', $item) }}">
                @csrf
                @method('PATCH')
    ...
    @if(in_array($item->status, ['submitted', 'reviewed'], true))
        <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <form method="POST" action="{{ route('advertiser.feedback.update', $item) }}" class="mt-4 grid gap-4 lg:grid-cols-2">
                @csrf
                @method('PUT')
                <div>
