...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('advertiser.campaigns.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-brand-600 text-sm font-semibold text-white hover:bg-brand-700 shadow-sm transition-colors">
    ...
    <div class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="px-5 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div class="flex items-center gap-1 overflow-x-auto">
                ...
                @endphp
                @foreach($tabs as $tab)
                    <a href="{{ route('advertiser.campaigns', ['status' => $tab['key'], 'search' => $search]) }}"
            ...
            {{-- Search --}}
            <form method="GET" action="{{ route('advertiser.campaigns') }}" class="relative">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-100">
                        ...
                        <th class="text-right px-5 py-3 text-[10px] font-semibold uppercase tracking-wider text-gray-400">Bid</th>
                        <th class="text-right px-5 py-3 text-[10px] font-semibold uppercase tracking-wider text-gray-400">Spend</th>
                        <th class="text-right px-5 py-3 text-[10px] font-semibold uppercase tracking-wider text-gray-400">Balance</th>
                    ...
                    @forelse($campaigns as $campaign)
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            ...
                            <td class="px-5 py-3.5 text-right tabular-nums">
                                <div class="font-semibold text-gray-900">&euro;{{ number_format($campaign['bid_amount'], 4) }}</div>
                                <div class="text-[10px] text-gray-400">{{ strtoupper($campaign['type']) }}</div>
                            </td>
                            {{-- Spend --}}
                            <td class="px-5 py-3.5 text-right tabular-nums">
                                <div class="font-semibold text-gray-900">&euro;{{ number_format($campaign['spend'], 2) }}</div>
                                @php $rowSpendPct = $campaign['budget'] > 0 ? round(($campaign['spend'] / $campaign['budget']) * 100, 1) : 0; @endphp
                                <div class="text-[10px] text-gray-400">{{ $rowSpendPct }}%</div>
                            </td>
                            {{-- Balance --}}
                            <td class="px-5 py-3.5 text-right tabular-nums">
                                <div class="font-semibold text-gray-900">&euro;{{ number_format($campaign['balance'], 2) }}</div>
                                <div class="text-[10px] text-gray-400">of &euro;{{ number_format($campaign['budget'], 2) }}</div>
                            </td>
                            {{-- Actions --}}
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-center gap-1">
                                    <a href="{{ route('advertiser.campaigns.show', $campaign['id']) }}" title="View Details" class="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors">
                                    ...
                                    <a href="{{ route('advertiser.campaigns.edit', $campaign['id']) }}" title="Edit Campaign" class="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors">
                                    ...
                                    @if($campaign['status'] === 'pending_review')
                                        <form method="POST" action="{{ route('advertiser.campaigns.updateStatus', $campaign['id']) }}" class="inline">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    <div class="relative" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.away="open = false"
                                            ...
                                            <a href="{{ route('advertiser.campaigns.show', $campaign['id']) }}" class="flex items-center gap-2 px-3 py-2 text-xs text-gray-600 hover:bg-gray-50">
                                            ...
                                            <form method="POST" action="{{ route('advertiser.campaigns.duplicate', $campaign['id']) }}" class="block">
                                                @csrf
                ...
                @if($pagination['total_pages'] > 1)
                    <div class="flex items-center gap-1">
                        ...
                        @if($pagination['current_page'] > 1)
                            <a href="{{ route('advertiser.campaigns', ['status' => $statusFilter, 'search' => $search, 'page' => $pagination['current_page'] - 1]) }}"
                        ...
                        @for($p = 1; $p <= $pagination['total_pages']; $p++)
                            ...
                            @else
                                <a href="{{ route('advertiser.campaigns', ['status' => $statusFilter, 'search' => $search, 'page' => $p]) }}"
    ...
    <div id="exportModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
            ...
            <div class="grid grid-cols-2 gap-3 mb-5">
                {{-- Excel --}}
                <a href="{{ route('advertiser.campaigns.export', ['format' => 'excel', 'status' => $statusFilter]) }}" class="flex flex-col items-center gap-2 p-4 rounded-xl border border-gray-200 hover:border-emerald-300 hover:bg-emerald-50 transition-colors group cursor-pointer">
                ...
                {{-- CSV --}}
                <a href="{{ route('advertiser.campaigns.export', ['format' => 'csv', 'status' => $statusFilter]) }}" class="flex flex-col items-center gap-2 p-4 rounded-xl border border-gray-200 hover:border-blue-300 hover:bg-blue-50 transition-colors group cursor-pointer">
    ...
    function openMoveToGroup(id, name) {
        ...
        document.getElementById('moveToGroupForm').action = '/advertisers/campaigns/' + id + '/group';
    ...
    function confirmDelete(id, name) {
        ...
        document.getElementById('deleteForm').action = '/advertisers/campaigns/' + id;
    ...
    function openPrintableView(isPdf) {
        ...
        html += '<table><thead><tr>';
        html += '<th>ID</th><th>Status</th><th>Campaign Name</th><th>Type</th><th>Start Date</th><th>End Date</th><th>Model</th><th class="text-right">Impressions</th><th class="text-right">Clicks</th><th class="text-right">Conversions</th><th class="text-right">CTR</th><th class="text-right">Bid</th><th class="text-right">Spend</th><th class="text-right">Balance</th>';
        html += '</tr></thead><tbody>';
        ...
        campaigns.forEach(function(c) {
            ...
            html += '<td class="text-right">€' + Number(c.bid_amount).toLocaleString(undefined, {minimumFractionDigits:4, maximumFractionDigits:4}) + '</td>';
            html += '<td class="text-right">€' + Number(c.spend).toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2}) + '</td>';
            html += '<td class="text-right">€' + Number(c.balance).toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2}) + '</td>';
            html += '</tr>';
        });
    ...
    function createGroup() {
        ...
        fetch('{{ route("advertiser.campaigns.groups.store") }}', {
            method: 'POST',
            headers: {
