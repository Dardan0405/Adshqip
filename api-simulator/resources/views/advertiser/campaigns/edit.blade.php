...
@section('content')
<form method="POST" action="{{ route('advertiser.campaigns.update', $campaign['id']) }}" enctype="multipart/form-data" id="editCampaignForm">
    @csrf
    @method('PUT')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <div class="flex items-center gap-2 text-sm text-gray-400 mb-1">
                <a href="{{ route('advertiser.campaigns') }}" class="hover:text-gray-600 transition-colors">Campaigns</a>
                ...
                <a href="{{ route('advertiser.campaigns.show', $campaign['id']) }}" class="hover:text-gray-600 transition-colors">#{{ $campaign['id'] }}</a>
                <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none"><path d="M9 5l7 7-7 7" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('advertiser.campaigns.show', $campaign['id']) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-200 bg-white text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">Cancel</a>
        ...
        <div class="p-6 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                    ...
                    <input type="number" name="total_budget" step="0.01" min="{{ $campaignSettings['minimum_budget'] }}" required value="{{ $campaign['budget'] }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                ...
                <div>
                    ...
                    <input type="number" name="bid_amount" step="0.0001" min="{{ $campaignSettings['minimum_bid_rate'] }}" value="{{ $campaign['bid_amount'] ?? '' }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <div class="p-4 bg-gray-50 rounded-lg border border-gray-200 text-center">
                    <p class="text-sm text-gray-500">No keywords available. <a href="{{ '#' }}" class="text-brand-600 hover:underline">Add keywords</a> first.</p>
                </div>
                @endif
                                ...
                                @foreach($campaign['ad_formats'] as $index => $af)
                                    <tr class="hover:bg-gray-50/50">
                                        <td class="px-4 py-2.5 text-sm font-medium text-gray-800">
                                            ...
                                            <input type="hidden" name="ad_formats[{{ $index }}][display_screen_id]" value="{{ $af['display_screen_id'] ?? '' }}">
                                            <input type="hidden" name="ad_formats[{{ $index }}][display_screen_name]" value="{{ $af['display_screen_name'] ?? '' }}">
    ...
    <div class="flex items-center justify-between py-4">
        <a href="{{ route('advertiser.campaigns.show', $campaign['id']) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-200 bg-white text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
...
function createPixel() {
    ...
    fetch('{{ route("advertiser.campaigns.pixels.store") }}', {
        method: 'POST',
        headers: {
