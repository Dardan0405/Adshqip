...
@section('content')
    ...
    <div class="space-y-6">
        <div class="relative overflow-hidden rounded-[28px] border border-slate-200 bg-gradient-to-br from-slate-950 via-slate-900 to-sky-900 px-6 py-7 text-white shadow-sm">
            ...
            <div class="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="max-w-2xl">
                    ...
                    <h1 class="mt-4 text-3xl font-semibold tracking-tight">Keep your personal details complete and ready across the publisher workspace.</h1>
                    <p class="mt-3 max-w-xl text-sm leading-6 text-slate-200/85">
                        Update the personal information shown for your publisher account, including your name, birth date, location, and mobile contact details.
                    </p>
                </div>
        ...
        <form method="POST" action="{{ route('publisher.personal-information.update') }}" class="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.4fr)_360px]">
            @csrf
            @method('PUT')
            ...
            <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="mb-6 flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">These details help identify the publisher account across the platform.</p>
                    </div>
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">Skype Address</label>
                        <input type="text" name="skype_address" value="{{ old('skype_address', $profile?->skype_address) }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-sky-400 focus:outline-none focus:ring-4 focus:ring-sky-100" placeholder="Enter Skype address">
                        @error('skype_address') <p class="mt-1.5 text-xs text-rose-500">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">ICQ Address</label>
                        <input type="text" name="icq_address" value="{{ old('icq_address', $profile?->icq_address) }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-sky-400 focus:outline-none focus:ring-4 focus:ring-sky-100" placeholder="Enter ICQ address">
                        @error('icq_address') <p class="mt-1.5 text-xs text-rose-500">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">European VAT</label>
                        <input type="text" name="vat_number" value="{{ old('vat_number', $profile?->vat_number) }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-sky-400 focus:outline-none focus:ring-4 focus:ring-sky-100" placeholder="Enter VAT number">
                        @error('vat_number') <p class="mt-1.5 text-xs text-rose-500">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">Jabber Address</label>
                        <input type="text" name="jabber_address" value="{{ old('jabber_address', $profile?->jabber_address) }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-sky-400 focus:outline-none focus:ring-4 focus:ring-sky-100" placeholder="Enter Jabber address">
                        @error('jabber_address') <p class="mt-1.5 text-xs text-rose-500">{{ $message }}</p> @enderror
                    </div>
                    <div>
            ...
            <div class="space-y-6">
                <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div class="rounded-3xl bg-slate-950 p-5 text-white">
                        <p class="text-xs uppercase tracking-[0.24em] text-slate-400">Publisher identity</p>
                    ...
                    <div class="mt-5 rounded-2xl bg-sky-50 p-4 text-sm text-sky-900">
                        Keep these fields current so your publisher record stays complete for future settings, approvals, and account-level activity.
                    </div>

                    <div class="mt-5">
                        <a href="{{ route('publisher.company-information') }}" class="inline-flex w-full items-center justify-center rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                            Company Information
                        </a>
                    </div>
                </section>
