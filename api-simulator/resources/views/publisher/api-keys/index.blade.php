...
@section('content')
    ...
    <div class="flex flex-col gap-2 mb-6">
        ...
        <p class="text-sm text-gray-500">Create and manage publisher integration keys for external services.</p>
    </div>
    ...
    <div class="grid gap-6 xl:grid-cols-[380px,1fr]">
        <div class="rounded-xl border border-gray-200 bg-white p-5 h-fit">
            ...
            <form method="POST" action="{{ route('publisher.api-keys.store') }}" class="space-y-4">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <div class="grid gap-2 sm:grid-cols-2 text-sm text-gray-600">
                        @foreach(['read_reports' => 'Read Reports', 'manage_campaigns' => 'Manage Campaigns', 'manage_sites' => 'Manage Sites', 'manage_adblocks' => 'Manage AdBlocks'] as $value => $label)
                        ...
                        <tr class="hover:bg-gray-50/50">
                            ...
                            <td class="px-4 py-3 text-right">
                                @if($apiKey->status === 'active')
                                    <form method="POST" action="{{ route('publisher.api-keys.revoke', $apiKey) }}" class="inline">
                                        @csrf
                                        @method('PATCH')
                                ...
                                @else
                                    <form method="POST" action="{{ route('publisher.api-keys.activate', $apiKey) }}" class="inline">
                                        @csrf
                                        @method('PATCH')
