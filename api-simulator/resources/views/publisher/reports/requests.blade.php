...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Daily ad request breakdown — impressions, clicks, conversions, and earnings.</p>
        </div>
    </div>
    ...
    {{-- Filters --}}
    <form method="GET" action="{{ route('publisher.reports.requests') }}" class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
            <div class="flex items-center gap-2 flex-1 flex-wrap">
                ...
                <a href="{{ route('publisher.reports.requests') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50">Reset</a>
            </div>
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    ...
                    <button type="button" onclick="copyRequestTable()" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Copy</button>
                    <a href="{{ route('publisher.reports.requests.export', request()->all()) }}" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">CSV</a>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-4 py-3 border-b border-gray-100">
            <h2 class="text-sm font-semibold text-gray-900">Daily Request Breakdown</h2>
            <p class="text-xs text-gray-500 mt-1">{{ $reports->total() }} days in selected period</p>
        </div>

        <div class="overflow-x-auto">
            <table id="requestReportTable" class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/50">
                        <th class="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-gray-400 whitespace-nowrap">Request</th>
                    ...
                    @forelse($reports as $row)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-4 py-3 whitespace-nowrap">
                                ...
                                <div class="text-xs text-gray-400">{{ $row->requests }} {{ Str::plural('request', $row->requests) }}</div>
                            </td>
                        ...
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-sm text-gray-500">No request data found for the selected period.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
@push('scripts')
<script>
    function copyRequestTable() {
        const rows = document.querySelectorAll('#requestReportTable tr');
        let text = '';
        rows.forEach(row => {
