...
@section('title', 'Adblock Report')

@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Adblock Report</h1>
            <p class="text-sm text-gray-500 mt-1">Aggregated earnings and traffic broken down by your ad zones.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
        @php
            $cards = [
                ['label' => 'Zones',       'value' => number_format($summary['total_zones']),       'color' => 'text-slate-900',   'bg' => 'bg-slate-50',   'border' => 'border-slate-200'],
    ...
    <form method="GET" action="{{ route('publisher.reports.adblocks') }}" class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
            <div class="flex items-center gap-2 flex-1 flex-wrap">
                <div class="relative flex-1 min-w-[180px] max-w-xs">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search zone name..."
                ...
                <a href="{{ route('publisher.reports.adblocks') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50">Reset</a>
            </div>
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    ...
                    <button type="button" onclick="copyAdblockTable()" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Copy</button>
                    <a href="{{ route('publisher.reports.adblocks.export', request()->all()) }}" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">CSV</a>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-4 py-3 border-b border-gray-100">
            <h2 class="text-sm font-semibold text-gray-900">Ad Zone Performance</h2>
            <p class="text-xs text-gray-500 mt-1">{{ $reports->total() }} zones in selected period</p>
        </div>

        <div class="overflow-x-auto">
            <table id="adblockReportTable" class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/50">
                        <th class="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-gray-400 whitespace-nowrap">Name</th>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-lg bg-brand-100 flex items-center justify-center text-brand-700 font-semibold text-xs flex-shrink-0">
                                        {{ strtoupper(substr($row->zone_name, 0, 2)) }}
                                    </div>
                                    <span class="font-medium text-gray-900">{{ $row->zone_name }}</span>
                                </div>
                            </td>
                        ...
                        <tr>
                            <td colspan="5" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    <svg class="w-12 h-12 text-gray-300" viewBox="0 0 24 24" fill="none"><path d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zm0 8a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zm12 0a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>
                                    <p class="text-sm text-gray-500">No adblock data found for the selected period.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
@push('scripts')
<script>
    function copyAdblockTable() {
        const rows = document.querySelectorAll('#adblockReportTable tr');
        let text = '';
        rows.forEach(row => {
