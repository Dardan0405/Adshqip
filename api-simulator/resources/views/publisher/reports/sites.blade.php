...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Site Report</h1>
            <p class="text-sm text-gray-500 mt-1">Earnings and traffic performance broken down by your sites.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
        @php
            $cards = [
                ['label' => 'Sites',       'value' => number_format($summary['total_sites']), 'color' => 'text-slate-900',   'bg' => 'bg-slate-50',   'border' => 'border-slate-200'],
    ...
    <form method="GET" action="{{ route('publisher.reports.sites') }}" class="bg-white rounded-xl border border-gray-200 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
            <div class="flex items-center gap-2 flex-1 flex-wrap">
                <div class="relative flex-1 min-w-[180px] max-w-xs">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search site or domain..."
                ...
                <a href="{{ route('publisher.reports.sites') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50">Reset</a>
            </div>
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    ...
                    <button type="button" onclick="copySiteTable()" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Copy</button>
                    <a href="{{ route('publisher.reports.sites.export', request()->all()) }}" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">CSV</a>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-4 py-3 border-b border-gray-100">
            <h2 class="text-sm font-semibold text-gray-900">Site Performance</h2>
            <p class="text-xs text-gray-500 mt-1">{{ $reports->total() }} sites in selected period</p>
        </div>

        <div class="overflow-x-auto">
            <table id="siteReportTable" class="w-full text-sm">
                <thead>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-lg bg-brand-100 flex items-center justify-center text-brand-700 font-semibold text-xs flex-shrink-0">
                                        {{ strtoupper(substr($row->site_name, 0, 2)) }}
                                    </div>
                                    <span class="font-medium text-gray-900">{{ $row->site_name }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3">
                                @if($row->site_url !== '#')
                                    <a href="{{ $row->site_url }}" target="_blank" rel="noopener noreferrer"
                        ...
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-sm text-gray-500">No site data found for the selected period.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
@push('scripts')
<script>
    function copySiteTable() {
        const rows = document.querySelectorAll('#siteReportTable tr');
        let text = '';
        rows.forEach(row => {
