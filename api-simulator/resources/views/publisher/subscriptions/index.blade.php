...
<div class="space-y-6">
    <div class="rounded-2xl border border-gray-200 bg-white p-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Subscription Plan</h1>
                <p class="mt-1 text-sm text-gray-500">Choose a plan managed in Admin Pricing Plans for publishers.</p>
            </div>
            <div class="inline-flex rounded-lg border border-gray-200 bg-white p-1 shadow-sm w-fit">
                <a href="{{ route('publisher.subscription-plan', ['cycle' => 'monthly']) }}" class="px-4 py-2 text-sm font-medium rounded-md {{ $cycle === 'monthly' ? 'bg-brand-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50' }}">Monthly</a>
                <a href="{{ route('publisher.subscription-plan', ['cycle' => 'yearly']) }}" class="px-4 py-2 text-sm font-medium rounded-md {{ $cycle === 'yearly' ? 'bg-brand-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50' }}">Yearly</a>
            </div>
        </div>
    </div>
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                @else
                    ...
                    <p class="mt-2 text-sm text-gray-500">Select a plan below to activate your subscription.</p>
                @endif
            </div>
            @if($currentSubscription && in_array($currentSubscription->status, ['active', 'trial'], true))
                <form method="POST" action="{{ route('publisher.subscription-plan.cancel', $currentSubscription) }}">
                    @csrf
                    @method('PATCH')
    ...
    @if($plans->isEmpty())
        <div class="rounded-lg border border-dashed border-gray-300 bg-white p-8 text-center">
            <h2 class="text-lg font-semibold text-gray-900">No publisher plans are active</h2>
            <p class="mt-2 text-sm text-gray-500">Create or unblock pricing plans in Admin Pricing Plans with target audience Publisher or Both.</p>
        </div>
    @else
        <div class="grid gap-5 xl:grid-cols-3 md:grid-cols-2">
            @foreach($plans as $plan)
                ...
                <div class="relative rounded-lg border bg-white p-5 shadow-sm {{ $plan->is_popular ? 'border-brand-300 ring-1 ring-brand-100' : 'border-gray-200' }}">
                    ...
                    <form method="POST" action="{{ route('publisher.subscription-plan.subscribe', $plan) }}" class="mt-5">
                        @csrf
