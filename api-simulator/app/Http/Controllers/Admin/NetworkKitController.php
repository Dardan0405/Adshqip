...
class NetworkKitController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.network-kit.index', [
            'summary' => $summary,
    ...
    public function export(Request $request)
    {
        ...
        $filename = 'network_kit_' . now()->format('Y-m-d_His') . '.csv';
        $headers = [
    ...
    private function baseQuery(array $filters)
    {
        ...
        if (!empty($filters['browser'])) {
            $query->where('stats.browser', strtolower($filters['browser']));
        }

        // Apply OS filter
        if (!empty($filters['os'])) {
            $query->where('stats.os', strtolower($filters['os']));
        }
    ...
    {
        // Get distinct values from database for dynamic filters
        ...
        // Get traffic sources (handle gracefully if table structure differs)
        try {
            $trafficSources = DB::table('aq_traffic_sources')
                ->select('id', 'name')
                ->orderBy('name')
                ->pluck('name', 'id')
                ->toArray();
        ...
        // Get adblocks/zones (handle gracefully if table structure differs)
        try {
            $adblocks = DB::table('aq_zones')
                ->select('id', 'name')
                ->orderBy('name')
                ->pluck('name', 'id')
                ->toArray();
