...
class CreativeReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Dropdowns
        $allAds         = Ad::where('is_deleted', false)->orderBy('name')->get(['id', 'name', 'campaign_id']);
        $allCampaigns   = Campaign::orderBy('name')->get(['id', 'name']);
        $allAdvertisers = User::where('role', 'advertiser')->with('userProfile')->orderBy('email')->get();

        return view('admin.creative-reports.index', compact(
            'reports',
            'totalClicks',
    ...
    public function export(Request $request)
    {
        ...
        $callback = function () use ($collection) {
            ...
            fputcsv($file, ['Period', 'Ad Name', 'Ad Type', 'Campaign', 'Advertiser Name', 'Advertiser Email', 'Impressions', 'Clicks', 'Conversions', 'Spend (€)', 'CTR (%)', 'ECPM (€)']);
            foreach ($collection as $row) {
                fputcsv($file, [
                    $row->month_formatted,
                    $row->ad_name,
