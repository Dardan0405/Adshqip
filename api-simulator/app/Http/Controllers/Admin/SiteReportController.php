...
class SiteReportController extends Controller
{
    public function index(Request $request)
    {
        $query = StatDaily::selectRaw("
                ...
                END as ecpm
            ")
            ->join('aq_sites', 'aq_stats_daily.site_id', '=', 'aq_sites.id')
            ->whereNotNull('aq_stats_daily.site_id')
            ->groupBy('month', 'aq_stats_daily.site_id', 'aq_sites.name', 'aq_sites.domain', 'aq_sites.status', 'aq_sites.publisher_id');
        ...
        if ($search = $request->get('search')) {
            $collection = $collection->filter(function ($item) use ($publishers, $search) {
                ...
                return stripos($item->site_name ?? '', $search) !== false
                    || stripos($item->site_domain ?? '', $search) !== false
                    || stripos($name, $search) !== false
                    || stripos($pub->email ?? '', $search) !== false;
            });
        }
        ...
        $activeSites            = $collection->pluck('site_id')->unique()->count();
        ...
        $allSites      = Site::where('is_deleted', false)->orderBy('name')->get(['id', 'name', 'domain']);
        $allPublishers = User::where('role', 'publisher')->with('userProfile')->orderBy('email')->get();

        return view('admin.site-reports.index', compact(
            'reports', 'totalClicks', 'totalUniqueImpressions', 'totalUniqueClicks',
            'totalConversions', 'totalImpressions', 'totalEarnings', 'activeSites',
            'allSites', 'allPublishers'
        ));
    }
    ...
    public function export(Request $request)
    {
        ...
        if ($s = $request->get('search')) { $collection = $collection->filter(fn($i) => stripos($i->site_name ?? '', $s) !== false || stripos($i->site_domain ?? '', $s) !== false); }
        ...
        return response()->stream(function () use ($collection) {
            ...
            fputcsv($f, ['Period', 'Site Name', 'Domain', 'Publisher Name', 'Publisher Email', 'Impressions', 'Clicks', 'Conversions', 'Earnings (€)', 'CTR (%)', 'ECPM (€)']);
            foreach ($collection as $r) {
                fputcsv($f, [$r->month_formatted, $r->site_name, $r->site_domain, $r->publisher_name, $r->publisher_email,
            ...
            fclose($f);
        }, 200, ['Content-Type' => 'text/csv', 'Content-Disposition' => 'attachment; filename="site_reports_' . date('Y-m-d_His') . '.csv"']);
    }
}
