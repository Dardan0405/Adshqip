...
class AdblockReportController extends Controller
{
    public function index(Request $request)
    {
        $query = StatDaily::selectRaw("
                DATE_FORMAT(aq_stats_daily.date, '%Y-%m')  as month,
                aq_stats_daily.zone_id,
                aq_zones.name                              as zone_name,
                aq_zones.format_key,
                aq_zones.status                            as zone_status,
                aq_zones.site_id,
                aq_sites.name                              as site_name,
                aq_sites.domain                            as site_domain,
                aq_sites.publisher_id,
            ...
            ->join('aq_sites', 'aq_zones.site_id', '=', 'aq_sites.id')
            ->whereNotNull('aq_stats_daily.zone_id')
            ->groupBy(
                'month', 'aq_stats_daily.zone_id',
                'aq_zones.name', 'aq_zones.format_key', 'aq_zones.status', 'aq_zones.site_id',
                'aq_sites.name', 'aq_sites.domain', 'aq_sites.publisher_id'
            );
        ...
        if ($search = $request->get('search')) {
            $collection = $collection->filter(function ($item) use ($publishers, $search) {
                ...
                return stripos($item->zone_name ?? '', $search) !== false
                    || stripos($item->site_name ?? '', $search) !== false
                    || stripos($item->site_domain ?? '', $search) !== false
                    || stripos($name, $search) !== false;
            });
        }

        if ($zoneId = $request->get('zone_id')) {
            $collection = $collection->filter(fn($i) => $i->zone_id == $zoneId);
        }

        if ($siteId = $request->get('site_id')) {
            $collection = $collection->filter(fn($i) => $i->site_id == $siteId);
        }
        ...
        $activeZones            = $collection->pluck('zone_id')->unique()->count();
        ...
        $allZones      = Zone::where('is_deleted', false)->orderBy('name')->get(['id', 'name', 'site_id', 'format_key']);
        $allSites      = Site::where('is_deleted', false)->orderBy('name')->get(['id', 'name']);
        $allPublishers = User::where('role', 'publisher')->with('userProfile')->orderBy('email')->get();

        return view('admin.adblock-reports.index', compact(
            'reports', 'totalClicks', 'totalUniqueImpressions', 'totalUniqueClicks',
            'totalConversions', 'totalImpressions', 'totalEarnings', 'activeZones',
            'allZones', 'allSites', 'allPublishers'
        ));
    }
    ...
    public function export(Request $request)
    {
        $rows = StatDaily::selectRaw("
                DATE_FORMAT(aq_stats_daily.date, '%Y-%m') as month,
                aq_stats_daily.zone_id,
                aq_zones.name as zone_name, aq_zones.format_key, aq_zones.site_id,
                aq_sites.name as site_name, aq_sites.domain as site_domain, aq_sites.publisher_id,
                SUM(aq_stats_daily.impressions) as total_impressions,
                SUM(aq_stats_daily.unique_impressions) as total_unique_impressions,
                SUM(aq_stats_daily.clicks) as total_clicks,
                SUM(aq_stats_daily.unique_clicks) as total_unique_clicks,
                ...
                CASE WHEN SUM(aq_stats_daily.impressions) > 0
                     THEN ROUND((SUM(aq_stats_daily.clicks)/SUM(aq_stats_daily.impressions))*100,2) ELSE 0 END as ctr,
                CASE WHEN SUM(aq_stats_daily.impressions) > 0
                     THEN ROUND((SUM(aq_stats_daily.publisher_earnings)/SUM(aq_stats_daily.impressions))*1000,2) ELSE 0 END as ecpm
            ")
            ->join('aq_zones', 'aq_stats_daily.zone_id', '=', 'aq_zones.id')
            ->join('aq_sites', 'aq_zones.site_id', '=', 'aq_sites.id')
            ->whereNotNull('aq_stats_daily.zone_id')
            ->groupBy('month', 'aq_stats_daily.zone_id', 'aq_zones.name', 'aq_zones.format_key', 'aq_zones.status', 'aq_zones.site_id', 'aq_sites.name', 'aq_sites.domain', 'aq_sites.publisher_id')
            ->get();

        $publishers = User::whereIn('id', $rows->pluck('publisher_id')->unique())->with('userProfile')->get()->keyBy('id');
        $collection = collect($rows->map(fn($r) => (object) $r->toArray()));

        if ($s = $request->get('search')) { $collection = $collection->filter(fn($i) => stripos($i->zone_name ?? '', $s) !== false || stripos($i->site_name ?? '', $s) !== false); }
        if ($id = $request->get('zone_id'))      { $collection = $collection->filter(fn($i) => $i->zone_id == $id); }
        if ($id = $request->get('site_id'))      { $collection = $collection->filter(fn($i) => $i->site_id == $id); }
        ...
        return response()->stream(function () use ($collection) {
            ...
            fputcsv($f, ['Period', 'AdBlock Name', 'Format', 'Site Name', 'Domain', 'Publisher Name', 'Publisher Email', 'Impressions', 'Clicks', 'Conversions', 'Earnings (€)', 'CTR (%)', 'ECPM (€)']);
            foreach ($collection as $r) {
                fputcsv($f, [$r->month_formatted, $r->zone_name, $r->format_key ?? '—', $r->site_name, $r->site_domain,
                    $r->publisher_name, $r->publisher_email,
            ...
            fclose($f);
        }, 200, ['Content-Type' => 'text/csv', 'Content-Disposition' => 'attachment; filename="adblock_reports_' . date('Y-m-d_His') . '.csv"']);
    }
}
