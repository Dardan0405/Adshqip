...
class PublisherSubscriptionPlanController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $plans = PricingPlan::query()
            ->active()
            ->whereIn('target_audience', ['publisher', 'both'])
            ->orderBy('sort_order')
            ->orderBy('id')
            ->get();
    ...
    public function subscribe(Request $request, PricingPlan $plan): RedirectResponse
    {
        ...
        if ($plan->status !== 'active' || ! in_array($plan->target_audience, ['publisher', 'both'], true)) {
            return back()->withErrors(['plan' => 'This subscription plan is not available for publishers.']);
        }
        ...
        if ($price !== null && (float) $price > 0 && $paymentType !== PlatformSetting::ADVERTISER_PAYMENT_WIRE_TRANSFER) {
            ...
            $returnUrl = route('publisher.subscription-plan.return', $createdSubscription) . '?provider=' . $paymentType;
            $cancelUrl = route('publisher.subscription-plan.payment-cancel', $createdSubscription);
        ...
        if ($price === null || (float) $price <= 0) {
            ...
            return redirect()
                ->route('publisher.subscription-plan', ['cycle' => $cycle])
    ...
    public function paymentReturn(Request $request, UserSubscription $subscription): RedirectResponse
    {
        ...
        try {
            if ($provider === PlatformSetting::ADVERTISER_PAYMENT_PAYPAL) {
                ...
                if ($subscription->status !== 'active' && ($invoice?->status ?? null) !== 'paid') {
                    return redirect()
                        ->route('publisher.subscription-plan')
                        ->with('success', 'Bitcoin payment submitted. The subscription activates after BitPay confirms it.');
                }
            }
        } catch (\RuntimeException $exception) {
            return redirect()
                ->route('publisher.subscription-plan', ['cycle' => $subscription->billing_cycle])
    ...
    {
        return 'PSUB-' . now()->format('YmdHis') . '-' . $userId . '-' . $planId . '-' . random_int(1000, 9999);
    }
}
