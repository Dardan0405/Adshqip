...
class CampaignController extends Controller
{
    ...
    {
        return $groupId === null || CampaignGroup::where('advertiser_id', auth()->id())->where('is_deleted', false)->whereKey($groupId)->exists();
    }

    private function ownsPixel(?int $pixelId): bool
    {
        return $pixelId === null || PixelTracker::where('advertiser_id', auth()->id())->where('is_deleted', false)->whereKey($pixelId)->exists();
    }

    /**
     * Display the current advertiser's campaign listing page.
     */
    public function index(Request $request)
    {
        ...
        $allCampaigns = $campaigns->map(function ($campaign) {
            ...
            $totalBudget = (float) ($campaign->total_budget ?? 0);
            $balance = $campaign->remaining_budget !== null
                ? (float) $campaign->remaining_budget
                : max(0, $totalBudget - $spend);
        ...
        $campaignGroups = CampaignGroup::where('advertiser_id', auth()->id())
            ->where('is_deleted', false)
            ->orderBy('name')
            ->get()
            ->map(function ($group) {
                return [
                    ...
                    'name' => $group->name,
                    'status' => $group->status,
                    'campaign_count' => $group->campaigns()
                        ->where('advertiser_id', auth()->id())
                        ->where('is_deleted', false)
                        ->count(),
                ];
        ...
        $pixelTrackers = PixelTracker::where('advertiser_id', auth()->id())
            ->where('is_deleted', false)
            ->orderBy('name')
            ->get()
            ->map(function ($pixel) {
    ...
    public function create()
    {
        ...
        $advertisers = User::where('id', auth()->id())
            ->with('profile')
            ->get()
            ->map(function ($adv) {
    ...
    public function store(Request $request)
    {
        ...
        if (!$this->ownsGroup(!empty($validated['group_id']) ? (int) $validated['group_id'] : null)) {
            return back()->withErrors(['group_id' => 'The selected campaign group is not available for your account.'])->withInput();
        }

        if (!$this->ownsPixel(!empty($validated['pixel_tracker_id']) ? (int) $validated['pixel_tracker_id'] : null)) {
            return back()->withErrors(['pixel_tracker_id' => 'The selected pixel tracker is not available for your account.'])->withInput();
        }
    ...
    {
        $campaign = Campaign::where('advertiser_id', auth()->id())->find($id);
    ...
    public function moveToGroup(Request $request, int $id)
    {
        ...
        $group = CampaignGroup::where('advertiser_id', auth()->id())->find($validated['group_id']);

        if (!$group) {
            return redirect()
                ->route('advertiser.campaigns')
                ->with('error', 'Campaign group not found');
        }
    ...
    public function show(int $id)
    {
        ...
        return view('advertiser.campaigns.show', ['campaign' => $campaignData]);
    }
    ...
    public function edit(int $id)
    {
        ...
        $campaign = Campaign::with(['group', 'zone.site'])
            ->where('advertiser_id', auth()->id())
            ->find($id);
    ...
    public function update(Request $request, int $id)
    {
        ...
        return redirect()
            ->route('advertiser.campaigns.show', $id)
