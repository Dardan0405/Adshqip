...
class TwoFactorAuthenticationController extends Controller
{
    ...
    public function show(Request $request, TwoFactorAuth $twoFactorAuth)
    {
        ...
        return view('publisher.two-factor-authentication', [
            'user'                       => $user,
    ...
    public function update(Request $request, TwoFactorAuth $twoFactorAuth)
    {
        ...
        if (in_array(TwoFactorAuth::METHOD_BACKUP, $tokenTypes, true) && blank($validated['two_factor_backup_question'] ?? null)) {
        ...
        if ($tokenTypes === []) {
            $this->notifier->deliver(
                ...
                'security_disable',
                'Security Disabled',
                'All two-factor authentication methods have been removed from your publisher account.',
                route('publisher.two-factor-authentication'),
                $user->id,
                true
            );
        } else {
