...
class AccountSettingsController extends Controller
{
    ...
    public function show(Request $request, TwoFactorAuth $twoFactorAuth)
    {
        ...
        return view('publisher.account-settings', [
            'user'            => $user,
    ...
    public function update(Request $request, TwoFactorAuth $twoFactorAuth, PublisherNotificationManager $notifier)
    {
        $user                = $request->user()->load('profile');
        ...
        DB::transaction(function () use ($request, $user, $validated) {
            ...
            if ($request->hasFile('avatar')) {
                if ($user->profile?->avatar_url) {
                    $existing = ltrim((string) str_replace('/storage/', '', parse_url($user->profile->avatar_url, PHP_URL_PATH) ?: $user->profile->avatar_url), '/');
                    if ($existing !== '') {
        ...
        DB::transaction(function () use ($request, $user, $twoFactorAuth, $enableTwoFactor, &$newBackupCodes) {
            ...
            if (blank($currentSecret)) {
                ...
                $user->forceFill(['two_factor_secret' => $twoFactorAuth->encryptSecret($currentSecret)])->save();
            }
        ...
        if ($wasTwoFactorEnabled && ! $enableTwoFactor) {
            $notifier->deliver(
                ...
                'security_disable',
                'Security Disabled',
                'Two-factor authentication has been disabled on your publisher account.',
                route('publisher.account-settings'),
                $user->id,
                true
            );
        } else {
